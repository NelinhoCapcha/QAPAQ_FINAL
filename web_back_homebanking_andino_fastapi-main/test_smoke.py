...
USER = os.getenv("HB_SMOKE_USER", "")
DNI = os.getenv("HB_SMOKE_DNI", "")
PWD = os.getenv("CLIENTE_PASSWORD", os.getenv("ALL_PASSWORD", ""))
...
def main():
    if not USER or not DNI or not PWD:
        raise RuntimeError("Define HB_SMOKE_USER, HB_SMOKE_DNI y CLIENTE_PASSWORD antes de ejecutar el smoke test.")

    # 0) Raíz
    show("GET /", client.get("/"))

    # 1) Login OK
    r = client.post("/auth/login", json={"username": USER, "dni": DNI, "password": PWD})
    ...
    show("POST /auth/login (bad pwd)",
         client.post("/auth/login", json={"username": USER, "dni": DNI, "password": "xxx"}))
