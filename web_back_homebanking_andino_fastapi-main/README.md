...
- **Login**: `username` = `codcliente` en minúscula, `dni` = documento del cliente y `password` = clave de internet.
- Las credenciales de prueba deben compartirse por un canal interno, no en documentación pública.
- Periodo de la cartera de créditos (`fagcuentacredito`): **202512**.

## Autenticación

`POST /auth/login` valida DNI, `activo='S'` y `bloqueado!='S'`, verifica el password con
...
curl -X POST http://localhost:8002/auth/login \
  ...
  -d '{"username":"<usuario>","dni":"<dni>","password":"<clave>"}'

http POST :8002/auth/login username=<usuario> dni=<dni> password=<clave>
```
